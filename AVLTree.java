...
class LUC_AVLTree {
    ...
    private Node deleteElement(int value, Node node) {
        ...
        // check if node passed as a param is null
        if (node != null) {
            if (value == node.value) { // Value found, at proper node to delete
                if (node.rightChild == null) {
                    if (node.leftChild == null) {
                        return null; // if no sub children -- return null
                    } else {
                        node = node.leftChild; // only left subtree (no right, means only left)
                    }
                } else {
                    if (node.leftChild == null) { // only right subtree (no left, means only right)
                        node = node.rightChild;
                    } else {
                        // Find in-order successor node (via minValueNode())
                        Node succ = minValueNode(node.rightChild);
                        // Copy in-order successor node's value to this node's value
                        node.value = succ.value;
                        // Delete the in-order successor node(via deleteElement())
                        node.rightChild = deleteElement(node.value, node.rightChild);
                    }
                } // check where to traverse (left or right)
            } else if (node.value > value) { // left
        ...
        // If this node's |bf| > 1, then rotation needed for this node
        // Hint: If node's bf > 1 (left heavy) then:
        // If bf of leftchild > 0 (left heavy)
        // Do LL else LR
        ...
        if (bf > 1)
        {
            ...
            {
                node = LLRotation(node); // LL rotation
            } else {
                node = LRRotation(node);
            }
        } else if(bf < -1) { // if right heavy
